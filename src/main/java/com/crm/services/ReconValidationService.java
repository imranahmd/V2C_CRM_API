...
@Service
public class ReconValidationService {
...
//				reconfileuploadData.setNoofException(0);
				...
				reconfileuploadData.setVersion("1.0");
						...
						//ValidReconDetails(reconConfigs, tblfileUploadstatus, filePath);
							...
							message = "1#"+tblfileUploadstatus.getFileId();
							 reconfileuploadData.setFileId(tblfileUploadstatus.getFileId());
								...
								tblFileUplaodRepo.save(reconfileuploadData);
							//HashMap<String, String> reconSummaryMap = reconManagementService.getReconSummary(tblFileupload.getFileId().toString(), tblFileupload.getFileName());
...
String extension = originalFileName.substring(originalFileName.lastIndexOf(".") + 1);
		...
		String serviceid = reconConfigs.getServiceId();
		...
		try {
			...
			transactionDate = new SimpleDateFormat("yyyy-MM-dd")
					.parse(reconConfigs.getDateformat());
		...
		logger.info("rows::::::::"+rows.toString());
	...
	//	reconManagementService.updateFileUpload(tblfileUploadstatus);
		...
		if (errorList.size() == 0 && fileAtrnString != null && fileAtrnString.length() > 0) {
			...
			ArrayList<String> fileAtrnList = new ArrayList<String>(
					Arrays.asList(fileAtrnString.split("','")));
		}
		...
		logger.info("tblfileUploadstatus::data::::"+rowValues.toString());
		logger.info("tblfileUploadstatus::columns::::"+columns);
		logger.info("tblfileUploadstatus::rows::::"+rows);
		logger.info("tblfileUploadstatus::columsMap::::"+columsMap);
		...
		int statusCode = reconManagementService.fileUploadReconNew(columns, rowValues, tblfileUploadstatus.getFileId(), isHeader, errorMap);
...
public String startRecon(String FileId) throws SQLException {
	...
	//	String query = "{call pro_ReconProcess()}";
		String query = "{call pro_ReconProcess_pa_v2_new(?)}";  // added for recon versioning
...
public String fileUploadPG(String validteReconInput )
{
	String message = "";
	ArrayList arrayList = new ArrayList();
	HashMap<String, String> list = new HashMap();
...
//			reconfileuploadData.setNoofException(0);
			reconfileuploadData.setNoofException("0");
			reconfileuploadData.setException("0");
			reconfileuploadData.setVersion("PG");
			ReconRecord tblfileUploadstatus = reconManagementService.saveFileUpload(reconfileuploadData);
			status = tblfileUploadstatus.getStatusCode();
			System.out.println("tblfileUploadstatus"+tblfileUploadstatus.getFileId()+" status: "+status);
			if (status == "1") {
...
@Async
public void ValidReconDetailsPG(TblMstreconconfig reconConfigs,ReconRecord tblfileUploadstatus,File filePath)
{
	ReconRecord reconfileuploadData = new ReconRecord();
String message=null;
String originalFileName = filePath.getName();
String extension = originalFileName.substring(originalFileName.lastIndexOf(".") + 1);
	...
	// Read File Content
	if (extension.equalsIgnoreCase("xls") || extension.equalsIgnoreCase("xlsx")) {
		rows = reconManagementService.readDataFromXLS(filePath, serviceName,serviceid);
	}else {
		rows = reconManagementService.readDataFromCSV(filePath, fileSeprator, serviceName);
	}
	...
	//System.out.println("isHeader===: "+isHeader);
	//System.out.println("errorMap===: "+errorMap);
	int statusCode = reconManagementService.fileUploadReconOld(columns, rowValues, tblfileUploadstatus.getFileId(), isHeader, errorMap);
...
@Async
public String startReconV1_Old(String FileId) throws SQLException {
	List<ReconRecord> recondata = new ArrayList<ReconRecord>();
	CallableStatement callableStatement = null;
	ResultSet rs = null;
	String result =null;
	Connection con = jdbcTemplate.getDataSource().getConnection();
	logger.info("call strored procedure------>pro_ReconProcessV1_PG()");
	try {
	//	String query = "{call pro_ReconProcessV1_PG()}";
		String query = "{call pro_ReconProcess_pa_v2_newRevamp(?)}";  // added for recon versioning---------pro_ReconProcessV1_PG_Revamp
